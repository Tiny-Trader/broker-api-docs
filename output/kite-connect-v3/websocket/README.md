...
# WebSocket streaming[¶](https://kite.trade/docs/connect/v3/websocket/#websocket-streaming "Permanent link")
...
In addition, the text messages, alerts, and order updates (the same as the ones available as [Postbacks](https://kite.trade/docs/connect/v3/postbacks/)) are also streamed. As the name suggests, the API uses
...
Note
Implementing an asynchronous WebSocket client with a binary parser for the market data structure may be a complex task. We recommend using one of our pre-built [client libraries](https://kite.trade/docs/connect/v3/#libraries-and-resources).
## Connecting to the WebSocket endpoint[¶](https://kite.trade/docs/connect/v3/websocket/#connecting-to-the-websocket-endpoint "Permanent link")
```
// Javascript example.
...
### Request structure[¶](https://kite.trade/docs/connect/v3/websocket/#request-structure "Permanent link")
```
...
### Modes[¶](https://kite.trade/docs/connect/v3/websocket/#modes "Permanent link")
...
## Binary market data[¶](https://kite.trade/docs/connect/v3/websocket/#binary-market-data "Permanent link")
...
For quote subscriptions, instruments are identified with their corresponding numerical `instrument_token` obtained from the [instrument list](https://kite.trade/docs/connect/v3/market-quotes/#retrieving-the-full-instrument-list) API.
### Message structure[¶](https://kite.trade/docs/connect/v3/websocket/#message-structure "Permanent link")
...
![WebSocket API message structure](https://kite.trade/docs/connect/v3/images/websocket-message.png)
|
---|---
...
### Quote packet structure[¶](https://kite.trade/docs/connect/v3/websocket/#quote-packet-structure "Permanent link")
...
4 - 8 | `int32` | Last traded price (_If mode is`ltp` , the packet ends here_)
...
40 - 44 | `int32` | Close price (_If mode is`quote` , the packet ends here_)
...
### Index packet structure[¶](https://kite.trade/docs/connect/v3/websocket/#index-packet-structure "Permanent link")
...
24 - 28 | `int32` | Price change (_If mode is`quote` , the packet ends here_)
28 - 32 | `int32` | Exchange timestamp
### Market depth structure[¶](https://kite.trade/docs/connect/v3/websocket/#market-depth-structure "Permanent link")
...
## Postbacks and non-binary updates[¶](https://kite.trade/docs/connect/v3/websocket/#postbacks-and-non-binary-updates "Permanent link")
Apart from binary market data, the WebSocket stream delivers postbacks and other updates in the text mode. These messages are JSON encoded and should be parsed on receipt. For order Postbacks, the payload is contained in the `data` key and has the same structure described in the [Postbacks](https://kite.trade/docs/connect/v3/postbacks/) section.
> Message structure
```
{
  "type": "order",
...
### Message types[¶](https://kite.trade/docs/connect/v3/websocket/#message-types "Permanent link")
type |
---|---
`order` | Order Postback. The `data` field will contain the full order [Postback](https://kite.trade/docs/connect/v3/postbacks/) payload
